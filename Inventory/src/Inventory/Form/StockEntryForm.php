<?php
	...
	class StockEntryForm extends Form {
		...
		public function __construct($name = null, AdapterInterface $dbAdapter, $postedValues) {
			...
			$this->add(array(
				...
				'attributes' => array(
					...
					'onblur'=> 'getSuppCOACodeAmount(this.value,1);',
		...
		public function getMaxInvoiceNo() {
			...
			$defaultInvoiceNo = 1;
			$maxInvoiceNo = '';
			$maxInvoiceSql = "SELECT MAX(substring(INVOICE_NO,17,5)) AS MAXINVOICENO FROM i_stock_details";
			...
			return 'INV/'.'PUR/'.$bYear.'/'.$bMonth.'/'.$defaultInvoiceNo;
		}
	}
?>	