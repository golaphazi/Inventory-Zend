<?php
	...
	class RETWiseStockDistributionForm extends Form {
		...
		public function __construct($name = null, AdapterInterface $dbAdapter, $postedValues) {
			...
			parent::__construct('retwisestockdistribution');
		...
		public function getMaxInvoiceNo() {
			...
			$defaultInvoiceNo = 1;
			$maxInvoiceNo = '';
			$maxInvoiceSql = "SELECT MAX(substring(INVOICE_NO,18,5)) AS MAXINVOICENO FROM i_retailer_stock_dist_details";
			...
			return 'INV/'.'SALE/'.$bYear.'/'.$bMonth.'/'.$defaultInvoiceNo;
		}
	}
?>	