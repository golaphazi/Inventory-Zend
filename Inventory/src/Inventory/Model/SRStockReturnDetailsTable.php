<?php
	...
	class SRStockReturnDetailsTable {
		...
		public function saveSRStockReturnDetails($srstockreturndetails) {
			...
			$rate		  	= '';
			$tcAmount		= '';
			for($i = 0; $i < sizeof($srstockreturndetails->CATEGORY_ID); $i++) {
				$modelID	= $srstockreturndetails->CATEGORY_ID[$i];
				if(isset($srstockreturndetails->CATEGORY_ID[$i])) {
					$qty		= $srstockreturndetails->SR_QUANTITY[$i];
					$rate		= $srstockreturndetails->SR_BUY_PRICE[$i];
					$tcAmount 	= $srstockreturndetails->SR_TOTAL_AMOUNT[$i];
					$data = array(
									'SR_STOCK_RETURN_ID' 	=> $srstockreturndetails->SR_STOCK_RETURN_ID,
									'CATEGORY_ID' 		=> $srstockreturndetails->CATEGORY_ID[$i],
									'CAT_PRICE_ID' 		=> $srstockreturndetails->CAT_PRICE_ID[$i],
									'SR_QUANTITY' 		=> str_replace(",", "", $srstockreturndetails->SR_QUANTITY[$i]),
									'SR_BUY_PRICE' 		=> str_replace(",", "", $srstockreturndetails->SR_BUY_PRICE[$i]),
									'SR_TOTAL_AMOUNT' 	=> str_replace(",", "", $srstockreturndetails->SR_TOTAL_AMOUNT[$i]),
									'SR_AVG_RATE' 		=> str_replace(",", "", $srstockreturndetails->SR_AVG_RATE[$i]),
									'SR_DISCOUNT' 		=> str_replace(",", "", $srstockreturndetails->SR_DISCOUNT[$i]),
									'SR_NET_AMOUNT' 	=> str_replace(",", "", $srstockreturndetails->SR_NET_AMOUNT[$i]),
									'STATUS' 		=> 'b',
									'SOLD_FLAG'	=> '',
					...
					if(!empty($recDate) 
					   ...
					   && !empty($tcAmount)) {
						...
						if($this->tableGateway->insert($data)) {
							...
							$compStockData = $this->fetchCompanyStockData($srstockreturndetails->CATEGORY_ID[$i]);
							$restOfQty = 0;
							$restOfQtyCheck = 0;
							$srQuantity = $srstockreturndetails->SR_QUANTITY[$i];
							foreach ($compStockData as $selectOption) {
								...
								if($compStockQty > $srQuantity) {
									$compStockTotalAmount	= $srQuantity * $srstockreturndetails->SR_BUY_PRICE[$i];
									$compStockNetAmount		= $compStockTotalAmount - $srstockreturndetails->SR_DISCOUNT[$i];
									$compStockBuyQty		= $compStockQty - $srQuantity;
									$compStockBuyTotalAmount	= $compStockBuyQty * $compStockBuyPrice;
									$compStockBuyNetAmount		= $compStockBuyTotalAmount - $srstockreturndetails->SR_DISCOUNT[$i];
										...
										$insertSaleQuery = "
															INSERT INTO 
																		...
																		VALUES
																			(
																				'".$compStockOderID."',
																				'".$srstockreturndetails->CATEGORY_ID[$i]."',
																				'".$compStockCatPriceID."',
																				'".$compStockSuppID."',
																				'".str_replace(",", "", $srQuantity)."',
																				'".str_replace(",", "", $srstockreturndetails->SR_BUY_PRICE[$i])."',
																				'".$compStockTotalAmount."',
																				'".str_replace(",", "", $compStockAvgRate)."',
																				'".str_replace(",", "", $srstockreturndetails->SR_DISCOUNT[$i])."',
									...
									$compStockTotalAmount		= $compStockQty * $srstockreturndetails->SR_BUY_PRICE[$i];
									$compStockNetAmount			= $compStockTotalAmount - $srstockreturndetails->SR_DISCOUNT[$i];
