<?php
	...
	use Inventory\Form\StockReturnForm;
	...
	class StockReturnController extends AbstractActionController {		
		protected $dbAdapter;
		...
		public function indexAction() {
			...
			$form 		= new StockReturnForm('stockreturn', $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter'), ($request->isPost()) ? $request->getPost() : array() );
				...
				//if($form->isValid()) {
					...
					for($i = 1; $i < $postedData->NumberOfRows; $i++) {
						...
						if(!empty($cOACodeData)) {
							...
							$stockData['RQUANTITY'][$j]					= $postedData["RQUANTITY{$i}"];
					...
					$stockOrderID = $this->getStockOrderTable()->saveStockReturnOrder($stockorder);
					if($stockOrderID > 0){
						$stockData['STOCK_ORDER_ID'] = $stockOrderID;
						$stockentry->exchangeArray($stockData);
						if($msg = $this->getStockEntryTable()->saveStockReturn($stockentry)) {
							$success = true;
							$msg = "Successfull";
						} else {
							$success = false;
					...
					for($o=0;$o<sizeof($SUPPLIER_DATAS);$o++) {
						...
						for($c=0;$c<sizeof($stockWiseVoucher[$SID]);$c++) {
							...
							$RECEIVABLE_COA_CODE		= $stockWiseVoucher[$SID][$c]['RECEIVABLE_COA_CODE'];
							$RECEIVABLE_COA_NAME		= $stockWiseVoucher[$SID][$c]['RECEIVABLE_COA_NAME'];
							...
							$INVESTMENT_DEBIT_CHECKED					= "";
							$INVESTMENT_CREDIT_CHECKED					= "checked='checked'";
							$INVESTMENT_DEBIT_DISABLED					= "disabled='disabled'";
							$INVESTMENT_CREDIT_DISABLED					= "";
							$INVESTMENT_AMOUNT_ALIGN					= "right";
							$INVESTMENT_VOUCHER_TYPE					= "C";
							$VOUCHER_COA_CODE[$SID]['COA_CODE'][]		= $INVESTMENT_COA_CODE;
							$VOUCHER_COA_CODE[$SID]['VOUCHER_TYPE'][]	= $INVESTMENT_VOUCHER_TYPE;
							$VOUCHER_COA_CODE[$SID]['VOUCHER_AMOUNT'][]	= abs($AMOUNT);
							$chartOfAccountDisplayInvestment	.= "
																	<tr class='{$oddClass}'>
																		<td align='left'>
																			{$INVESTMENT_COA_NAME}
						...
						if($DISCOUNT_TYPE == 'individual'){
							$TOTAL_PAYABLE_AMOUNT	= $NETTOTAL_HIDDEN;
						} else if($postedData["DISCOUNT_TYPE"] == 'intotal') {
							$TOTAL_PAYABLE_AMOUNT	= $NETTOTAL_HIDDEN - abs($TOTAL_DISCOUNT_AMOUNT);
						}
						
						$PAYABLE_DEBIT_CHECKED						= "checked='checked'";
						$PAYABLE_CREDIT_CHECKED						= "";
						$PAYABLE_DEBIT_DISABLED						= "";
						$PAYABLE_CREDIT_DISABLED					= "disabled='disabled'";
						$PAYABLE_AMOUNT_ALIGN						= "left";
						$PAYABLE_VOUCHER_TYPE						= "D";
						...
						$PARTICULARS_INVESTMENT	 = "Stock Return ".$PARTICULARS_INVESTMENT." on ".$businessDate." to ".$SUPP_SHORT_NAME."";
						$chartOfAccountDisplay	.= "
														<tr class='oddClass'>
						...
						if($stockDetails['NET_PAYMENT'] > 0){
							$chartOfAccountPayment	= "
													<table cellspacing='0' class='vtbl' >
														<tr class='head'>
															<td colspan='4' align='center'>
																Receipt Voucher for Supplier : {$SUPP_SHORT_NAME}
															</td>
														</tr>
														<tr class='head'>
															<td width='55%' align='left'>Account Head</td>
							...
							$PAYMENT_COA_NAME = $RECEIVABLE_COA_NAME;
							$PAYMENT_COA_CODE = $RECEIVABLE_COA_CODE;
							$PAYMENT_DEBIT_CHECKED						= "";
							$PAYMENT_CREDIT_CHECKED						= "checked='checked'";
							$PAYMENT_DEBIT_DISABLED						= "disabled='disabled'";
							$PAYMENT_CREDIT_DISABLED					= "";
							$PAYMENT_AMOUNT_ALIGN						= "right";
							$PAYMENT_VOUCHER_TYPE						= "C";
							...
							$PAYMENT_PARTICULARS = 	"Received from {$SUPP_SHORT_NAME} for the purpose of stock return on ".$businessDate."";
							$chartOfAccountPayment	.= "
															<tr class='oddClass'>
																<td align='left'>Particulars</td>
																<td align='left' colspan='3'>
					...
					if($success) {
						...
						return $this->redirect()->toRoute('stockreturn');
					} else {
		...
		public function getSuggestRefCOANameAction() {
			...
			if(!empty($input)){
				$investorInfoArray 	= array();
					$IPAData = $this->getStockEntryTable()->fetchStockWiseModelDetails($input,$suppInfoId);	
					$catID = '';		
					foreach ($IPAData as $selectOption) {
						$catID = $selectOption['CATEGORY_ID'];
						$catName = $selectOption['CATEGORY_NAME'];				
						$catNameID = $catID.",".$catName;
						$sumOfQuantity = $this->getStockEntryTable()->fetchSumofStockQuantityForReturn($catID);
						$totalSumOfQuantity = '';
						...
						//$str .= "<div align='left' onClick=\"fill_id('".$catNameID."','".$no."','".$buyPrice."','".$catPriceID."','".$totalSumOfQuantity."','".$coaCode."','".$coaName."');\"><b>".$catName."</b></div>";
					}
				if(strlen($catID)>0){
						//$str .= $catIDName;
						$str = $catNameID.",".$no.",".$buyPrice.",".$catPriceID.",".$totalSumOfQuantity.",".$coaCode.",".$coaName;
						//$str = 1;
					}else{
						$str = 0;
					}
		...
		*/
		...
		public function getSuppCOACodeAction() {
			...
			foreach ($IPAData as $selectOption) {
				...
				$receivableCoaName = $selectOption['RECEIVABLE_COA_NAME'];
				$suppDetails_array[] = array('payableCoa'=>$payableCoa,'payableCoaName'=>$payableCoaName,'suppShortName'=>$suppShortName,'receivableCoa'=>$receivableCoa,'receivableCoaName'=>$receivableCoaName);
			}
