<?php
	...
	use Inventory\Form\RETWiseStockDistributionForm;
	...
	class RETWiseStockDistributionController extends AbstractActionController {		
		protected $dbAdapter;
		...
		public function indexAction() {
			...
			$form 		= new RETWiseStockDistributionForm('retstockentry', $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter'), ($request->isPost()) ? $request->getPost() : array() );
				...
				//if($form->isValid()) {
					...
					for($i = 1; $i < $postedData->NumberOfRows; $i++) {
						...
						if(!empty($cOACodeData)) {
							...
							$retstockData['RET_QUANTITY'][$j]				= str_replace(",", "", $postedData["QUANTITY{$i}"]);
							...
							$retstockData['REMARKS'][$j]					= str_replace(",", "", $postedData["REMARKS{$i}"]);
							$retstockData['RET_NET_AMOUNT'][$j]				= str_replace(",", "", $postedData["NET_AMOUNT{$i}"]);
							$retstockData['STOCK_BUY_PRICE'][$j]			= str_replace(",", "", $postedData["STOCK_BUY_PRICE{$i}"]);
							$stockTotalBuyAmount 							= $retstockData['STOCK_BUY_PRICE'][$j] * $retstockData['RET_QUANTITY'][$j];
							...
							$stockDetails = array(
													...
													"STOCKTOTALBUYAMOUNT"	=> $stockTotalBuyAmount,
													
												);
							array_push($stockWiseVoucher[$RETAILER_ID],$stockDetails);
						}
					}
					if(empty($stockDetails['RECEIVABLE_COA_CODE'])){
						$success = false;
						$msg = "Chart of account not yet set for respective retailer !";
						break;
					}
					//echo '<pre>';print_r($stockDetails);die();
					...
					$retstockorderData['RET_DISCOUNT_AMOUNT']				= str_replace(",", "", $postedData["INTOTALDISCOUNT"]);//str_replace(",", "", $postedData["TOTALDISCOUNT_HIDDEN"]);					
					$retstockorderData['RET_TOT_DIS_RECEIVE']				= str_replace(",", "", $postedData["TOTALDISCOUNTRECEIVE_HIDDEN"]);
					$retstockorderData['RET_LESS_DESCRIPTION']				= $postedData["RET_LESS_DESCRIPTION"];
					$retstockorder->exchangeArray($retstockorderData);
					//echo '<pre>';print_r($retstockorder);die();
					$retStockOrderID = $this->getRETStockOrderTable()->saveRETStockOrder($retstockorder);
					if($retStockOrderID > 0){
						$retstockData['RET_STOCK_DIST_ID']					= $retStockOrderID;
						$retstockData['ORDER_NO']							= $postedData["ORDER_NO"];
						$retstockData['INVOICE_NO']							= $postedData["INVOICE_NO"];
						$retstockdetails->exchangeArray($retstockData);
						//echo '<pre>';print_r($retstockdetails);die();
						if($msg = $this->getRETStockDetailsTable()->saveRETStockDetails($retstockdetails)) {							
							$paymenttransaction = new PaymentTransaction();
							$paymenttransactionData = array();
							$paymenttransactionData['ZONE_ID'] 				= $postedData["ZONE_ID"];
							$paymenttransactionData['EMPLOYEE_ID'] 			= $postedData["EMPLOYEE_ID"];
							$paymenttransactionData['RETAILER_ID'] 			= $postedData["RETAILER_ID"];
							$paymenttransactionData['TRANSACTION_FLAG'] 	= 'sale';
					...
					for($o=0;$o<sizeof($RETAILER_DATAS);$o++) {
						...
						for($c=0;$c<sizeof($stockWiseVoucher[$RID]);$c++) {
							...
							$STOCKTOTALBUYAMOUNT	= $stockWiseVoucher[$RID][$c]['STOCKTOTALBUYAMOUNT'];
							...
							$TOTAL_GAINLOSS_AMOUNT						+= ($AMOUNT - $STOCKTOTALBUYAMOUNT);
							...
							$VOUCHER_COA_CODE[$RID]['VOUCHER_AMOUNT'][]	= abs($STOCKTOTALBUYAMOUNT);
							...
							$chartOfAccountDisplayEncashment	.= "
																	<tr class='{$oddClass}'>
																		...
																		<td align='{$ENCASHMENT_AMOUNT_ALIGN}'>
																			<input type='text' name='PAYMENT_AMOUNT[$voucherTypeIndex]' value=".number_format(abs($STOCKTOTALBUYAMOUNT),4)." style='font-family:Tahoma, Geneva, sans-serif;text-align:right;width:120px'>
																		</td>
																	</tr>
																	";
							$voucherTypeIndex++;
						...
						///capital gain loss start
						if($TOTAL_GAINLOSS_AMOUNT > 0){
							$GAINLOSS_DEBIT_CHECKED						= "";
							$GAINLOSS_CREDIT_CHECKED					= "checked='checked'";
							$GAINLOSS_DEBIT_DISABLED					= "disabled='disabled'";
							$GAINLOSS_CREDIT_DISABLED					= "";
							$GAINLOSS_AMOUNT_ALIGN						= "right";
							$GAINLOSS_VOUCHER_TYPE						= "C";	
							$VOUCHER_COA_CODE[$RID]['COA_CODE'][]		= $GAINLOSS_COA_CODE;
							$VOUCHER_COA_CODE[$RID]['VOUCHER_TYPE'][]	= $GAINLOSS_VOUCHER_TYPE;
							$VOUCHER_COA_CODE[$RID]['VOUCHER_AMOUNT'][]	= abs($TOTAL_GAINLOSS_AMOUNT);
							...
							$chartOfAccountDisplayEncashment	.= "
																	<tr class='{$oddClass}'>
																		...
																		<td align='center'>																												
																			<input type='hidden' name='COA_CODE[]' value='{$GAINLOSS_COA_CODE}'/>
																			{$GAINLOSS_COA_CODE}
																		</td>
																		<td align='center'>
																		  <input type='radio' name='VOUCHER_TYPE[$voucherTypeIndex]' value='D' {$GAINLOSS_DEBIT_CHECKED} {$GAINLOSS_DEBIT_DISABLED}/>Dr 
																		  <input type='radio' name='VOUCHER_TYPE[$voucherTypeIndex]' value='C' {$GAINLOSS_CREDIT_CHECKED} {$GAINLOSS_CREDIT_DISABLED}/>Cr
																		</td>
																		<td align='{$GAINLOSS_AMOUNT_ALIGN}'>
																			<input type='text' name='PAYMENT_AMOUNT[$voucherTypeIndex]' value=".number_format(abs($TOTAL_GAINLOSS_AMOUNT),4)." style='font-family:Tahoma, Geneva, sans-serif;text-align:right;width:120px'>
																		</td>
																	</tr>
																	";
							
							///capital gain loss End
							$voucherTypeIndex++;
						} else {
							$GAINLOSS_DEBIT_CHECKED						= "checked='checked'";
							$GAINLOSS_CREDIT_CHECKED					= "";
							$GAINLOSS_DEBIT_DISABLED					= "";
							$GAINLOSS_CREDIT_DISABLED					= "disabled='disabled'";
							$GAINLOSS_AMOUNT_ALIGN						= "left";
							$GAINLOSS_VOUCHER_TYPE						= "D";	
							$VOUCHER_COA_CODE[$RID]['COA_CODE'][]		= $GAINLOSS_COA_CODE;
							$VOUCHER_COA_CODE[$RID]['VOUCHER_TYPE'][]	= $GAINLOSS_VOUCHER_TYPE;
							$VOUCHER_COA_CODE[$RID]['VOUCHER_AMOUNT'][]	= abs($TOTAL_GAINLOSS_AMOUNT);
							$chartOfAccountDisplayEncashment	.= "
						...
						$TOTAL_RECEIVABLE_AMOUNT						= 0.00;
						$TOTAL_RECEIVABLE_AMOUNT						= $TOTAL_SALE_AMOUNT - $TOTAL_DISCOUNT_AMOUNT;
						...
						$PARTICULARS_ENCASHMENT	 = "Stock Sell ".$PARTICULARS_ENCASHMENT." on ".$businessDate." to ".$RET_SHORT_NAME."";
						$chartOfAccountDisplay	.= "
														<tr class='oddClass'>
						...
						if($stockDetails['NET_PAYMENT'] > 0){
							...
							$RECEIVE_PARTICULARS = 	"Received from {$RET_SHORT_NAME} for the purpose of sales on ".$businessDate."";
							$chartOfAccountReceive	.= "
															<tr class='oddClass'>
																<td align='left'>Particulars</td>
																<td align='left' colspan='3'>
														...
														";
							//payment receive insert into payment transaction table starts here //
							$paymenttransaction = new PaymentTransaction();
							$paymenttransactionData = array();
							$paymenttransactionData['ZONE_ID'] 				= $postedData["ZONE_ID"];
							$paymenttransactionData['EMPLOYEE_ID'] 			= $postedData["EMPLOYEE_ID"];
							$paymenttransactionData['RETAILER_ID'] 			= $postedData["RETAILER_ID"];
							$paymenttransactionData['TRANSACTION_FLAG'] 	= 'receive';
							$paymenttransactionData['BRANCH_ID'] 			= $BRANCH_ID;
					...
					if($success) {
						...
						$msg = "Successfull". '&nbsp;Print Invoice&nbsp;<a onclick="if(confirm(\'Are you sure you want to print this invoice now?\')){return true;} else {return false;};" href="/branchvoucherprint/saleinvoiceprint?invoiceNo='.$postedData["INVOICE_NO"].'&invoiceType=sale" target="_blank">Click Here</a>';
						...
						return $this->redirect()->toRoute('retstockentry');
					} else {
			...
			if(!empty($input)){
				...
				foreach ($IPAData as $selectOption) {
					...
					if(strlen($catId)>0){
						...
						$str = $catIDName.",".$no.",".$sellPrice.",".$catPriceID.",".$totalSumOfQuantity.",".$coaCode.",".$coaName.",".$stockBuyPrice;
						
					}else{
						$str = 0;
					}
					//$str .= "<div align='left' onClick=\"fill_id('".$catIDName."','".$no."','".$sellPrice."','".$catPriceID."','".$totalSumOfQuantity."','".$coaCode."','".$coaName."','".$stockBuyPrice."');\"><b>".$catName."</b></div>";
				}
		...
		*/
		...
		public function getRetCOACodeAction() {
			...
			$branchID = $branchid;
			$branchData  = $this->getCompBranchTable()->getBranchInfoFinancialStatement($branchID);
			...
			$debitOp 				= 0;
			$creditOp 				= 0;
			$debitOpeningBalance1 	= 0;
			$creditOpeningBalance1 	= 0;
			$opBalance1 			= 0;
			$bDrCr2 				= '';
			$balance 				= 0;
			...
			foreach ($IPAData as $selectOption) {
				...
				$openingBalance  = $this->getTrialBalanceTable()->fetchOpeningBalance($receivableCoa,$fromDate,$branchID,$branchName);
				if($closingBalance >0)
				{
					$bDrCr1 = '';
					if($ob = ($balance1>0)){
						$bDrCr1 = 'Dr';
						$debitOpeningBalance1 = $balance1+$debitOpeningBalance1;
					}
					else{
						$bDrCr1 = 'Cr';
						$creditOpeningBalance1 = $balance1+$creditOpeningBalance1;
					}
					$totalBalance = abs($debitOpeningBalance1) - abs($creditOpeningBalance1);
					if($totalBalance == 0) {
						$bDrCr2 = '&nbsp;';
				...
				$cond = "AND ACCOUNT_CODE_T = '".$receivableCoa."' AND DATE_FTMY BETWEEN '".date("Y-m-d", strtotime($transDateFrom))."' AND '".date("Y-m-d", strtotime($transDateTo))."' ORDER BY RECORD_DATE ASC";
				...
				$opBalance = 0;
				$sv = 1;
				$viewLedgerInfo  = $this->getTrialBalanceTable()->fetchFromViewLedger($branchName,$cond);
				foreach ($viewLedgerInfo as $viewLedgerInfoData) :
					$debit	  			= $viewLedgerInfoData['DEBIT_'];
					$credit 			= $viewLedgerInfoData['CREDIT_'];
					if(!empty($debit)){
						$debitAmount = $debit;
						$balance += $debitAmount;
						$debitOp = $debitOp + $debitAmount;
						$debitOpeningBalance1+=$debitAmount;
						if($balance > 0){
							$debCre = "Dr";
						}else{
							$debCre = "Cr";
					...
					elseif(!empty($credit)){
						...
						$creditOp = $creditOp + $creditAmount;
						$creditOpeningBalance1-=$creditAmount;
						if($balance > 0){
							$debCre = "Dr";
						}else{
							$debCre = "Cr";
				...
				$retDetails_array[] = array(
											...
											'receivableCoaName'=>$receivableCoaName,
											'retShortName'=>$retShortName,
											'receivableAmount'=>number_format(abs($totalBalance),2)
											);
			}
		...
		public function getSRCurrentBalanceAction() {
			$this->session = new SessionContainer('post_supply');
			$businessDate = $this->session->businessdate;
			$branchid = 1;//$this->session->branchid;
			$srID = $_REQUEST['srID'];
			...
			$cond = '';			
			$srDetails_array = array();
			$balance 				= 0.00;
			$balancePlace			= '';			
			$balance = $this->getVoucherTable()->getSRCurrentBalance($srID,$fromDate,$branchID);	
			if($balance > 0){
				$balancePlace = number_format($balance,2);
			} else {
				$balancePlace = '('.number_format(abs($balance),2).')';
			}
			$srDetails_array[] = array('receivableAmount'=> $balancePlace);
			echo json_encode($srDetails_array);
			//echo $payableCoa;
			exit;
		}
		public function getRetCurrentBalanceAction() {
			$this->session = new SessionContainer('post_supply');
			$businessDate = $this->session->businessdate;
			$branchid = 1;//$this->session->branchid;
			$retID = $_REQUEST['retID'];
			...
			foreach ($IPAData as $selectOption) {
				...
				$receivableCoaName = $selectOption['COA_NAME'];
				$retShortName = $selectOption['SHOP_NAME'];
				$balance = $this->getVoucherTable()->getRetailerCurrentBalance($retID,$fromDate,$branchID);	
				if($balance > 0){
					$balancePlace = number_format($balance,2);
				} else {
					$balancePlace = '('.number_format(abs($balance),2).')';
			...
			$retDetails_array[] = array('receivableCoa'=>$receivableCoa,
										'receivableCoaName'=>$receivableCoaName,
										'retShortName'=>$retShortName,
										'receivableAmount'=> $balancePlace);
			echo json_encode($retDetails_array);
			//echo $payableCoa;
			exit;
		}
		public function getCompBranchTable() {
			if(!$this->compBranchTable) {
				$sm = $this->getServiceLocator();
				$this->compBranchTable = $sm->get('CompanyInformation\Model\BranchTable');
			}
			return $this->compBranchTable;
		}
