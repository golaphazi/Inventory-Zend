<?php
	...
	use Inventory\Form\SRWiseStockReturnForm;
	use Accounts\Model\TrialBalanceTable;
	
	use Inventory\Model\SRStockReturn;
	use Inventory\Model\SRStockReturnDetails;
	
	use Zend\Session\Container as SessionContainer;
	
	class SRWiseStockReturnController extends AbstractActionController {		
		protected $dbAdapter;
		protected $portfolioTrialBalanceTable;
		protected $branchTable;
		protected $coaTable;
		protected $trialBalanceTable;
		protected $srstockReturnTable;
		...
		public function indexAction() {
			...
			$form 		= new SRWiseStockReturnForm('srstockreturn', $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter'), ($request->isPost()) ? $request->getPost() : array() );
			$form->get('submit')->setValue('Add');
			if($request->isPost()) {
				$srstockdetails = new SRStockReturnDetails();
				...
				//if($form->isValid()) {
					...
					$srstockreturn = new SRStockReturn();
					$srstockreturnData = array();
					$srstockreturnData['ORDER_NO'] 							= $postedData["ORDER_NO"];
					$srstockreturnData['EMPLOYEE_ID'] 						= $postedData["EMPLOYEE_ID"];
					$srstockreturnData['SR_TOTAL_AMOUNT']					= str_replace(",", "", $postedData["TOTALAMOUNT_HIDDEN"]);
					$srstockreturnData['SR_DISCOUNT_AMOUNT']					= str_replace(",", "", $postedData["TOTALDISCOUNT_HIDDEN"]);
					$srstockreturn->exchangeArray($srstockreturnData);
					$srStockReturnID = $this->getSRStockReturnTable()->saveSRStockReturn($srstockreturn);
					if($srStockReturnID > 0){
						$srstockData['SR_STOCK_DIST_ID']					= $srStockReturnID;
						$srstockdetails->exchangeArray($srstockData);
						if($msg = $this->getSRStockReturnDetailsTable()->saveSRStockReturnDetails($srstockdetails)) {
							$success = true;
							$msg = "Successfull";
						} else {
							$success = false;
						...
						$success = false;
						throw new \Exception("Error: Cannot add stock return into system!");
					}
					if($success) {
		...
		public function getSuggestRefCOANameAction() {
			...
			$IPAData = $this->getSRStockReturnDetailsTable()->fetchCatFromSRStock($input);	
			$coaCode = '';		
			foreach ($IPAData as $selectOption) {
				$coaCode = $selectOption['CATEGORY_ID'];
				$coaHead = $selectOption['CATEGORY_NAME'];				
				$coaCodeHead = $coaCode.",".$coaHead;
				$sumOfQuantity = $this->getSRStockReturnDetailsTable()->fetchSumofStockQuantityFromSR($coaCode);
				$totalSumOfQuantity = '';
				foreach ($sumOfQuantity as $sumOfQuantityData) {
					$totalSumOfQuantity = $sumOfQuantityData['SRQUANTITY'];
		...
		public function getSRStockReturnTable() {
			if(!$this->srstockReturnTable) {
				$sm = $this->getServiceLocator();
				$this->srstockReturnTable = $sm->get('Inventory\Model\SRStockReturnTable');
			}
			return $this->srstockReturnTable;
		}
		public function getSRStockReturnDetailsTable() {
			if(!$this->srstockDetailsTable) {
				$sm = $this->getServiceLocator();
				$this->srstockDetailsTable = $sm->get('Inventory\Model\SRStockReturnDetailsTable');
			}
