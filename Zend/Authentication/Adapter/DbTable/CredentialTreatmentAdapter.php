...
class CredentialTreatmentAdapter extends AbstractAdapter
{
    ...
    protected function authenticateCreateSelect()
    {
		...
		$dbSelect->from(array('OPT'=>$this->tableName))
            ->columns(array('*', $credentialExpression))
			->join(array('USR'=>'up_is_user'),'OPT.USER_ID = USR.USER_ID','USER_NAME','inner')
			->join(array('USRRL'=>'up_is_user_role'),'USR.USER_ID = USRRL.USER_ID','ROLE_ID','inner')
			->join(array('RL'=>'s_role'),'USRRL.ROLE_ID = RL.ROLE_ID',array('ROLE_NAME', 'FORWARD_TO'),'inner')
            ->where(new SqlOp($this->identityColumn, '=', $this->identity))
			->where(new SqlOp('USR.USER_STATUS', '=', 'active'))
			->where(array('USRRL.END_DATE' => null));
