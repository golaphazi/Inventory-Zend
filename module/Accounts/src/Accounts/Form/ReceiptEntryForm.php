<?php
	...
	class ReceiptEntryForm extends Form {
		protected $dbAdapter;
		protected $postedValues;
		
		public function __construct($name = null, AdapterInterface $dbAdapter, $postedValues) {
			//parent::__construct($name);
			parent::__construct('receiptentry');
			$this->setAttribute('method','post');
			$this->setAttribute('onsubmit','return doValidationReceiptEntry();');
		...
		public function getMaxInvoiceNo() {
			...
			$defaultInvoiceNo = 1;
			$maxInvoiceNo = '';
			$maxInvoiceSql = "SELECT MAX(substring(INVOICE_NO,17,5)) AS MAXINVOICENO FROM a_transaction_master WHERE substring(INVOICE_NO,5,3) = 'REE'";
			...
			return 'INV/'.'REE/'.$bYear.'/'.$bMonth.'/'.$defaultInvoiceNo;
		}
	}
?>	