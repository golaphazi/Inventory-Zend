<?php
	...
	use Accounts\Form\ReceiptEntryForm;
	...
	class ReceiptEntryController extends AbstractActionController {
		protected $dbAdapter;
		protected $coaTable;
		...
		public function indexAction() {
			...
			$form 		= new ReceiptEntryForm('receiptentry', $this->getServiceLocator()->get('Zend\Db\Adapter\Adapter'), ($request->isPost()) ? $request->getPost() : array() );
			...
			if($request->isPost()) {
				...
				$receiveFrom		= $postedData["RECEIVE_FROM"];
				$backDateFlag		= '';
					...
					if($returnTransactionNo=$this->getMasterTable()->saveMaster($master)) {
						for($i=0;$i<sizeof($tcAccCodes);$i++) {
							...
							if($this->getChildTable()->saveChild($child)) {	
								...
								} else if($receiveFrom == 'rfs'){
									$SUPPLIER_INFO_ID = $this->getSupplierInformationTable()->fetchSupplierIDForPaymentReceipt($tcAccCodes[$i],'receivable');	
									$paymenttransaction = new PaymentTransaction();
				...
				if($success) {
					...
					if($success) {
						$this->flashmessenger()->addMessage("<table align='center' cellpadding='2' cellspacing='2' border='0' width='100%' style='font-size:100%;'>
																<tr class='valid_msg'>
																	<td width='100%' style='text-align:center;'>Receipt entry successfully saved.".$msg.'&nbsp;<a onclick="if(confirm(\'Are you sure you want to print this vourcher no. '.substr($msg,-16).' \')){return true;} else {return false;};" href="/branchvoucherprint/branchvoucherprint?tranDateFrom=&tranDateTo=&vType=&voucherData='.substr($msg,-16).'&branchID='.$postedData["BRANCH_ID"].'" target="_blank">Print this Voucher</a>'."</td>																
																</tr>
															</table>");
						return $this->redirect()->toRoute('receiptentry');
					} else {
						$this->flashmessenger()->addMessage("<table align='center' cellpadding='2' cellspacing='2' border='0' width='100%' style='font-size:100%;'>
															<tr class='error_msg'>
																<td width='100%' style='text-align:center;'>Receipt entry couldn't saved.".$msg.'&nbsp;<a onclick="if(confirm(\'Are you sure you want to print this vourcher no. '.substr($msg,-16).' \')){return true;} else {return false;};" href="/branchvoucherprint/branchvoucherprint?tranDateFrom=&tranDateTo=&vType=&voucherData='.substr($msg,-16).'&branchID='.$postedData["BRANCH_ID"].'" target="_blank">Print this Voucher</a>'."</td>																
															</tr>
														</table>");
						return $this->redirect()->toRoute('receiptentry');
					}
