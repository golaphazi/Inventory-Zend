	...
	class RetailerInformationTable {
		...
		public function getSRWiseRETList($id) {
			$select = "		
						...
						OR		ls_sr_retailer_map.END_DATE = 'Active')
		...
		public function getRetailerListForMap($id) {
			...
			*/
			...
			if($resultSet){
				...
				$result .='<tr><td width="5%"><input type="checkbox" name="chargeAllCheck" onclick="check(\'investor_charge\')" id="investor_charge_all" value="" disabled="disabled"  /></td>
						 <td width="35%"><b>SHOP</b></td><td width="30%" style="text-align:left;"><b>RET. Name</b></td><td width="30%" style="text-align:left;"><b>ADDRESS</b></td>
						</tr>';
				foreach($resultSet as $row) {						
						...
						$selectSR = "		
									SELECT 
											 ls_sr_retailer_map.EMPLOYEE_ID, ls_sr_retailer_map.RETAILER_ID
									FROM 
											ls_sr_retailer_map
									WHERE
											ls_sr_retailer_map.EMPLOYEE_ID = {$id}
											 AND ls_sr_retailer_map.RETAILER_ID = {$RETAILER_ID}
											 AND (ls_sr_retailer_map.END_DATE = '0000-00-00'
											 OR ls_sr_retailer_map.END_DATE = 'Active'
											 )
									
						";
						$stmt1 = $this->tableGateway->getAdapter()->createStatement($selectSR);
						$stmt1->prepare();
						...
						foreach($resultSet1 as $row1){
							$emID = $row1['RETAILER_ID'];
						}
						if($emID == $RETAILER_ID){
							$result .='<td><input type="checkbox" name="RETAILER_ID[]" checked="checked" onclick="check_me_add('.$i.','.$RETAILER_ID.','.$id.');" id="investor_charge'.$i.'" value="'.$RETAILER_ID.'" /></td>';
						}else{
							$result .='<td><input type="checkbox" name="RETAILER_ID[]" onclick="check_me_add('.$i.','.$RETAILER_ID.','.$id.');" id="investor_charge'.$i.'" value="'.$RETAILER_ID.'"  /></td>';
						}
						$result .='<td> '.$SHOP_NAME.'</td><td> '.$NAME.'</td><td> '.$ADDRESS.'</td>';
						$result .='</tr>';
					$i++;
					}
					$result .='</table>';
		...
		public function fetchRetailerListforLedger($input,$cond) {
			if(!empty($input)){
				$getTblDataSql   = "SELECT 
								...
								AND 	LOWER(RETINFO.SHOP_NAME) = '".$input."' {$cond}
